...
const StoryModel = ({ setShowModel, fetchStories }) => {
  ...
  const handleCreateStory = async () => {
    // Example stub: replace with API/Firebase call
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (text || media) {
          resolve("ok");
        } else {
          reject(new Error("Please add text or media"));
        }
      }, 1500);
    });
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex gap-2 mt-4">
          <button
            ...
            className={`flex-1 flex items-center cursor-pointer gap-2 p-2 rounded ${mode === "text" ? "bg-white text-black" : "bg-zinc-800"}`}
          >
        ...
        <button
          ...
          className="flex items-center w-full justify-center gap-2 py-3 mt-4 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95 transition text-white cursor-pointer"
        >
